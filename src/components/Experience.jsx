        ...
        <ul className='flex flex-col  justify-center'>
          <li className="mt-10">
            ...
            <div className="w-full h-1 bg-gradient-to-r from-gray-500 to-gray-100 mt-6 mb-6 custom-animation"></div>
                ...
                <div className='flex items-center justify-between'>
              <div>
                <div className='mt-4 text-sm font-secondary text-black font-medium'> I was a part of the VMWare Cloud Foundation Business Unit, where I worked in the ESXi LifeCycle Team, extensively VMware HostProfiles.</div>
              </div>
              <div >
                {/* Toggle visibility and change the icon based on state */}
                {isTextVisible ? (
                  <FaSquareMinus onClick={toggleTextVisibility} size={20} />
                ) : (
                  <FaSquarePlus onClick={toggleTextVisibility} size={20} />
                )}
              </div>
            </div>


            {/* Render text conditionally based on state */}
            {isTextVisible && (
              <div className={`transition-height overflow-hidden ${isTextVisible ? 'h-auto' : 'h-0'}`}>
